...
easywsclient::WebSocket::pointer from_url(const std::string& url, bool useMask, const std::string& origin) {
    ...
    else if (sscanf_s(url.c_str(), "ws://%[^:/]:%d/%s", host,128, &port, path,128) == 3) 
	{
    }
    else if (sscanf_s(url.c_str(), "ws://%[^:/]/%s", host,128, path,128) == 2) 
	{
        port = 80;
    }
    else if (sscanf_s(url.c_str(), "ws://%[^:/]:%d", host,128, &port) == 2) 
	{
        path[0] = '\0';
    }
    else if (sscanf_s(url.c_str(), "ws://%[^:/]", host,128) == 1) 
	{
        port = 80;
        path[0] = '\0';
    }
    ...
    }
    {
        ...
        int status;
        int i;
        _snprintf_s(line, 256, "GET /%s HTTP/1.1\r\n", path); ::send(sockfd, line, strlen(line), 0);
        if (port == 80) {
			_snprintf_s(line, 256, "Host: %s\r\n", host); ::send(sockfd, line, strlen(line), 0);
        }
        else {
			_snprintf_s(line, 256, "Host: %s:%d\r\n", host, port); ::send(sockfd, line, strlen(line), 0);
        }
		_snprintf_s(line, 256, "Upgrade: websocket\r\n"); ::send(sockfd, line, strlen(line), 0);
		_snprintf_s(line, 256, "Connection: Upgrade\r\n"); ::send(sockfd, line, strlen(line), 0);
        if (!origin.empty()) {
			_snprintf_s(line, 256, "Origin: %s\r\n", origin.c_str()); ::send(sockfd, line, strlen(line), 0);
        }
		_snprintf_s(line, 256, "Sec-WebSocket-Key: x3JJHMbDL1EzLkh9GBhXDw==\r\n"); ::send(sockfd, line, strlen(line), 0);
		_snprintf_s(line, 256, "Sec-WebSocket-Version: 13\r\n"); ::send(sockfd, line, strlen(line), 0);
		_snprintf_s(line, 256, "\r\n"); ::send(sockfd, line, strlen(line), 0);
        for (i = 0; i < 2 || (i < 255 && line[i-2] != '\r' && line[i-1] != '\n'); ++i) { if (recv(sockfd, line+i, 1, 0) == 0) { return NULL; } }
        line[i] = 0;
        if (i == 255) { fprintf(stderr, "ERROR: Got invalid status line connecting to: %s\n", url.c_str()); return NULL; }
        if (sscanf_s(line, "HTTP/1.1 %d", &status,sizeof(status)) != 1 || status != 101) { fprintf(stderr, "ERROR: Got bad status connecting to %s: %s", url.c_str(), line); return NULL; }
